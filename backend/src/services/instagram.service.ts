...
/**
 * Real posts from @attume.official, transcribed from the account so the grid
 * shows the studio's own words while no API token is configured. Images cannot
 * be used here: Instagram's CDN links are signed, expire, and block hotlinking,
 * so each card is typographic and opens the post on Instagram instead.
 */
